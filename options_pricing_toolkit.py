...
def bachelier_price(spec: OptionSpec) -> float:
    ...
    discounted_gap = math.exp(-r * T) * (S * math.exp(r * T) - K)

    if stdev == 0:
        if spec.option_type == "call":
            return max(discounted_gap, 0.0)
        return max(-discounted_gap, 0.0)

    d = discounted_gap / (math.exp(-r * T) * stdev)
    call = math.exp(-r * T) * (
        discounted_gap / math.exp(-r * T) * normal_cdf(d) + stdev * normal_pdf(d)
    )

    if spec.option_type == "call":
        return call
    return call - math.exp(-r * T) * (S * math.exp(r * T) - K)
...
def plot_volatility_sensitivity(spec: OptionSpec, vol_grid: np.ndarray) -> None:
    bs_prices = []
    bach_prices = []
    ...
    plt.plot(vol_grid, bs_prices, label="Black-Scholes", linewidth=2)
    plt.plot(vol_grid, bach_prices, label="Bachelier", linewidth=2)
    plt.xlabel("Volatility")
    plt.ylabel("Option Price")
    plt.title(f"Volatility Sensitivity, {spec.option_type.capitalize()} Option")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
...
def plot_terminal_distribution(spec: OptionSpec, n_paths: int = 50_000) -> None:
    terminal = simulate_terminal_prices_gbm(
        spot=spec.spot,
        maturity=spec.maturity,
        rate=spec.rate,
